import sys, io, json, ssl, urllib.request
...
# Scene presets from scene-presets.ts
PRESETS = [
    {
        "scene_id": "general",
        "label": "기본 공장",
        ...
        "chat_messages": [
            ...
            {"from": "ai", "text": "⚠ 사출기 #4 베어링 마모 감지\n위험도: 82%\n권고: 3일 내 점검 필요", "alert": True, "at": 0.5},
    ...
    },
    {
        ...
        "chat_messages": [
            ...
            {"from": "ai", "text": "⚠ AOI 불량률: 4.2% (기준 1.5%)\n주요 불량: 솔더 브릿지\n원인 추정: 리플로우 온도 편차", "alert": True, "at": 0.5},
        ...
        "overlay_texts": [
            ...
            {"title": "AOI 불량률 급증! 4.2%", "subtitle": "솔더 브릿지 · 리플로우 온도 편차"},
    ...
    },
    {
        ...
        "chat_messages": [
            ...
            {"from": "ai", "text": "⚠ CNC-3 스핀들 진동: 0.8mm/s (기준 0.3mm/s)\n베어링 마모 확률: 76%\n정밀도 저하 위험", "alert": True, "at": 0.5},
        ...
        "overlay_texts": [
            ...
            {"title": "스핀들 진동 이상! 0.8mm/s", "subtitle": "베어링 마모 76% · 정밀도 저하 위험"},
    ...
    },
    {
        ...
        "chat_messages": [
            ...
            {"from": "ai", "text": "⚠ 서버랙-1 온도: 42°C (기준 35°C)\nCPU 사용률: 94%\n냉각 팬 RPM 저하 의심", "alert": True, "at": 0.5},
        ...
        "overlay_texts": [
            ...
            {"title": "서버 온도 급상승! 42°C", "subtitle": "CPU 94% · 냉각 팬 RPM 저하"},
...
# RLS 때문에 임시로 비활성화 필요 — 또는 service_role key 사용
# 여기서는 anon key + RLS public read로 INSERT는 안 됨
# 마이그레이션 SQL로 직접 넣겠음
print("Generating SQL insert for scene_presets...")
...
for p in PRESETS:
    vals = {
        ...
        "equipment": json.dumps(p["equipment"], ensure_ascii=False),
        "stickers": json.dumps(p["stickers"]),
        "alert_index": p["alert_index"],
        "waypoints": json.dumps(p["waypoints"]),
        "kiosk_pos": json.dumps(p["kiosk_pos"]),
        "camera_keyframes": json.dumps(p["camera_keyframes"], ensure_ascii=False),
        "chat_messages": json.dumps(p["chat_messages"], ensure_ascii=False),
        "overlay_texts": json.dumps(p["overlay_texts"], ensure_ascii=False),
        "sort_order": p["sort_order"],
    }

    sql = f"""INSERT INTO scene_presets (scene_id, label, icon, description, equipment, stickers, alert_index, waypoints, kiosk_pos, camera_keyframes, chat_messages, overlay_texts, sort_order) VALUES (
  '{vals["scene_id"]}', '{vals["label"]}', '{vals["icon"]}', '{vals["description"]}',
  '{vals["equipment"]}'::jsonb, '{vals["stickers"]}'::jsonb, {vals["alert_index"]},
  '{vals["waypoints"]}'::jsonb, '{vals["kiosk_pos"]}'::jsonb,
  '{vals["camera_keyframes"]}'::jsonb, '{vals["chat_messages"]}'::jsonb, '{vals["overlay_texts"]}'::jsonb, {vals["sort_order"]}
) ON CONFLICT (scene_id) DO UPDATE SET
  label=EXCLUDED.label, equipment=EXCLUDED.equipment, stickers=EXCLUDED.stickers,
  alert_index=EXCLUDED.alert_index, waypoints=EXCLUDED.waypoints, kiosk_pos=EXCLUDED.kiosk_pos,
  camera_keyframes=EXCLUDED.camera_keyframes, chat_messages=EXCLUDED.chat_messages, overlay_texts=EXCLUDED.overlay_texts;"""
    sql_lines.append(sql)

with open("supabase/migrations/20260409000002_seed_presets.sql", "w", encoding="utf-8") as f:
    f.write("\n\n".join(sql_lines))

print(f"Generated {len(sql_lines)} preset inserts")
